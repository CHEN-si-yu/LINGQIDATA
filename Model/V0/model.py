...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=4)  # batch包含的天数
    parser.add_argument('--weight_decay', type=float, default=2e-2)  # weight_decay
    ...
    parser.add_argument('--max_epochs', type=int, default=1)  # 最大轮数
    parser.add_argument('--min_epochs', type=int, default=1)  # 最小轮数，防止早停过早结束
...
import pathlib
_DRIVE = pathlib.Path(__file__).resolve().drive
# 模型文件存储目录
root_path = rf'{_DRIVE}/BaiduSyncdisk/Model/V0'
# 因子
fac_path = rf'{_DRIVE}/BaiduSyncdisk/lingqingData/trainingdata/V0/'
fac_name = rf'fac20260510'
# 标签
label_path = rf'{_DRIVE}/BaiduSyncdisk/lingqingData/trainingdata/V0/'
label_name = rf'label'
# 流动性数据
liquid_path = rf'{_DRIVE}/BaiduSyncdisk/lingqingData/trainingdata/V0/'
...
'''训练验证测试部分相关设定要求：主要是val_wei以及学习率的调度'''
class DLLitModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # datamodule只是按照批次并行读取了数据，但是在实际处理过程中仍旧是一个截面一个截面地进行处理
    ...
    def configure_optimizers(self):
        ...
        scheduler = ReduceLROnPlateau(
            optimizer, 
            mode='max',          # 监控的指标模式（'max' 表示指标越大越好）
            factor=0.5,         # 学习率衰减因子（每次调整为原来的 0.5 倍）
            patience=3,         # 容忍的 epoch 数（如果 5 个 epoch 指标没有提升，则降低学习率）
            min_lr=5e-6,         # 学习率下限
            cooldown=2           # 下降后冷却两个epoch不会继续下降
        )
        optim_config = {
            'optimizer': optimizer,
            'lr_scheduler': {
                'scheduler': scheduler,
                'monitor': 'val_wei',  # 监控的指标（与 validation_step 中的 self.log('val_wei') 对应）
            },
        }
        return optim_config
...
def train_single(args, name, seed, train_date_list, valid_date_list, test_date_list):
    ...
    trainer_callbacks = [TQDMProgressBar(refresh_rate=1)] if enable_tqdm_progress else []
...
def train(args, name, market, season, fold, state='train'):
    ...
    def get_train_date_split(kfold ,date_list , season , period , interval=10 , month=24):
        ...
        # 2024国庆后的下跌行情归入2024q3的测试集以及2024q4训练集
        if season == '2024q3':
        ...
        if args.shuffle_split:
            # 获取 train+valid 区间（提前 interval 天以防泄露）
