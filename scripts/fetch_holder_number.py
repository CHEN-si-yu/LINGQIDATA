...
def _fetch_range(start_time, end_time, api_key):
    ...
    if start_time == end_time:
        log_print(f"  [holder_number] {start_time} total={total} > 100K, "
    ...
    if mid == start_dt:
        log_print(f"  [holder_number] {start_time}~{end_time} total={total} > 100K, "
                  f"fetching first 100K rows")
        all_data = list(page0)
        page = 1
        while page * 10000 <= 100000:
    ...
    log_print(f"  [holder_number] splitting {start_time}~{end_time} "
...
def fetch_holder_number(start_date="2019-01-01", end_date=None, output=None,
                        resume=True, workers=6, cleanup=True):
    """Fetch shareholder count (holder_num) for all stocks, quarterly."""
    ...
    if not months:
        log_print("[holder_number] No months in range")
        return pd.DataFrame()

    log_print(f"[holder_number] {len(months)} months: "
              f"{months[0][0]} ~ {months[-1][1]}")

    checkpoints_dir = Path(DATA_DIR) / "holder_number"
    ...
    for (r_start, r_end) in months:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[holder_number] {label} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[holder_number] {label} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append((r_start, r_end, label))

    if not todo:
        log_print(f"[holder_number] All {len(months)} months already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[holder_number] {len(todo)} to fetch, workers={workers}, "
                ...
                for fut in as_completed(futures):
                    ...
                    try:
                        ...
                        with lock:
                            ...
                            completed += 1
                        log_print(f"[holder_number] [{completed}/{len(todo)}] {label} -> "
                                  f"{len(batch)} rows | tokens={limiter.stats['tokens_available']}")
                    except Exception as e:
                        with lock:
                            completed += 1
                        log_print(f"[holder_number] [{completed}/{len(todo)}] {label} FAILED: {e}")
        else:
            for i, (rs, re, label) in enumerate(todo):
                batch = _fetch_and_save(rs, re, label, api_key, checkpoints_dir)
                if batch:
                    all_data.extend(batch)
                log_print(f"[holder_number] [{i+1}/{len(todo)}] {label} -> "
                          f"{len(batch)} rows | tokens={limiter.stats['tokens_available']}")
    ...
    if not df.empty and 'stock_code' in df.columns:
        ...
        if len(df) < before:
            log_print(f"[holder_number] Dropped {before - len(df)} rows with malformed stock_code")

        _key = df['stock_code'].str.extract(r'(\d{6})').iloc[:, 0]
        df['_sort_code'] = pd.to_numeric(_key, errors='coerce').fillna(0).astype(int)
        df = df.sort_values(['_sort_code', 'end_date']).drop(columns=['_sort_code']).reset_index(drop=True)

    log_print(f"[holder_number] Total: {len(df)} rows, {len(df.columns)} columns")

    if output is None:
        output = f"{DATA_DIR}/holder_number.parquet"
    df.to_parquet(output, index=False)
    log_print(f"[holder_number] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[holder_number] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch shareholder count data")
