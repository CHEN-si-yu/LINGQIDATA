...
def fetch_limit_list(start_date="2019-01-01", end_date=None, output=None,
                     resume=True, workers=6, cleanup=True):
    """Fetch limit-up/down list for all stocks."""
    ...
    if not months:
        log_print("[limit_list] No months in range")
        return pd.DataFrame()

    log_print(f"[limit_list] {len(months)} months: "
              f"{months[0][0]} ~ {months[-1][1]}")

    checkpoints_dir = Path(DATA_DIR) / "limit_list"
    ...
    for (r_start, r_end) in months:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[limit_list] {label} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[limit_list] {label} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append((r_start, r_end, label))

    if not todo:
        log_print(f"[limit_list] All {len(months)} months already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[limit_list] {len(todo)} to fetch, workers={workers}, "
    ...
    if not df.empty and 'stock_code' in df.columns:
        ...
        if len(df) < before:
            log_print(f"[limit_list] Dropped {before - len(df)} rows with malformed stock_code")
    ...
    log_print(f"[limit_list] Total: {len(df)} rows, {len(df.columns)} columns")

    if output is None:
        output = f"{DATA_DIR}/limit_list.parquet"
    df.to_parquet(output, index=False)
    log_print(f"[limit_list] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[limit_list] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch limit-up/down list")
