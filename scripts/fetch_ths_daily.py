...
def _fetch_range(start_time, end_time, api_key):
    """Fetch all rows, splitting if total exceeds 100K."""
    ...
    log_print(f"  [ths_daily] splitting {start_time}~{end_time} "
...
def fetch_ths_daily(start_date="2019-01-01", end_date=None, output=None,
                    resume=True, workers=6, cleanup=True):
    """Fetch THS (同花顺) index daily K-line data for all indices."""
    ...
    if not months:
        log_print("[ths_daily] No months in range")
        return pd.DataFrame()

    log_print(f"[ths_daily] {len(months)} months: "
              f"{months[0][0]} ~ {months[-1][1]}")

    checkpoints_dir = Path(DATA_DIR) / "ths_daily_chk"
    ...
    for (r_start, r_end) in months:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[ths_daily] {label} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[ths_daily] {label} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append((r_start, r_end, label))

    if not todo:
        log_print(f"[ths_daily] All {len(months)} months already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[ths_daily] {len(todo)} to fetch, workers={workers}, "
                ...
                for fut in as_completed(futures):
                    ...
                    try:
                        ...
                        with lock:
                            ...
                            completed += 1
                        log_print(f"[ths_daily] [{completed}/{len(todo)}] {label} -> "
                                  f"{len(batch)} rows | tokens={limiter.stats['tokens_available']}")
                    except Exception as e:
                        with lock:
                            completed += 1
                        log_print(f"[ths_daily] [{completed}/{len(todo)}] {label} FAILED: {e}")
        else:
            for i, (rs, re, label) in enumerate(todo):
                batch = _fetch_and_save(rs, re, label, api_key, checkpoints_dir)
                if batch:
                    all_data.extend(batch)
                log_print(f"[ths_daily] [{i+1}/{len(todo)}] {label} -> "
                          f"{len(batch)} rows | tokens={limiter.stats['tokens_available']}")

    df = pd.DataFrame(all_data)

    if not df.empty:
        if 'ths_code' in df.columns and 'trade_date' in df.columns:
            before = len(df)
            df = df[df['ths_code'].notna()]
            if len(df) < before:
                log_print(f"[ths_daily] Dropped {before - len(df)} rows with null ths_code")
            df = df.sort_values(['ths_code', 'trade_date']).reset_index(drop=True)

    log_print(f"[ths_daily] Total: {len(df)} rows, {len(df.columns)} columns")

    if output is None:
        output = f"{DATA_DIR}/ths_daily.parquet"
    df.to_parquet(output, index=False)
    log_print(f"[ths_daily] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[ths_daily] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch THS index daily K-line data")
