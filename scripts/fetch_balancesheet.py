...
def fetch_balancesheet(start_date="2019-01-01", end_date=None, output=None,
    ...
    if not quarters:
        log_print("[balancesheet] No quarters in range")
        return pd.DataFrame()

    log_print(f"[balancesheet] {len(quarters)} quarters: {quarters[0]} ~ {quarters[-1]}")

    checkpoints_dir = Path(DATA_DIR) / "balancesheet"
    ...
    for q_end in quarters:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[balancesheet] {q_end} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[balancesheet] {q_end} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append(q_end)

    if not todo:
        log_print(f"[balancesheet] All {len(quarters)} quarters already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[balancesheet] {len(todo)} to fetch, workers={workers}, "
                ...
                for fut in as_completed(futures):
                    ...
                    try:
                        ...
                        with lock:
                            all_data.extend(batch)
                            completed += 1
                        log_print(f"[balancesheet] [{completed}/{len(todo)}] {q} -> "
                                  f"{len(batch)} rows | tokens={limiter.stats['tokens_available']}")
                    except Exception as e:
                        with lock:
                            completed += 1
                        log_print(f"[balancesheet] [{completed}/{len(todo)}] {q} FAILED: {e}")
        else:
            for i, q_end in enumerate(todo):
                batch = _fetch_and_save(q_end, api_key, checkpoints_dir)
                if batch:
                    all_data.extend(batch)
                log_print(f"[balancesheet] [{i+1}/{len(todo)}] {q_end} -> "
                          f"{len(batch)} rows | tokens={limiter.stats['tokens_available']}")

    df = pd.DataFrame(all_data)

    # Drop rows with malformed stock_code (keep only 6-digit.XX format)
    if not df.empty and 'stock_code' in df.columns:
        before = len(df)
        df = df[df['stock_code'].str.match(r'^\d{6}\.(SH|SZ|BJ)$')]
        if len(df) < before:
            log_print(f"[balancesheet] Dropped {before - len(df)} rows with malformed stock_code")
    ...
    log_print(f"[balancesheet] Total: {len(df)} rows, {len(df.columns)} columns")

    if output is None:
        output = f"{DATA_DIR}/balancesheet.parquet"
    df.to_parquet(output, index=False)
    log_print(f"[balancesheet] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[balancesheet] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch stock balancesheet statement data")
