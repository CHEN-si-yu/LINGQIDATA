...
def _month_label(start_str, end_str):
    """Short label for checkpoint filename, e.g. '2020-01'."""
    return start_str[:7]


def _fetch_range(start_time, end_time, api_key):
    """Fetch all rows for a date range.

    If the result set exceeds the API pagination limit (100 000 rows),
    split the range in half and recurse.  Otherwise paginate normally.
    """
    ...
    log_print(f"  [finance] splitting {start_time}~{end_time} "
...
def _fetch_and_save(start_time, end_time, label, api_key, checkpoints_dir):
    """Fetch one month (adaptively) and atomically save checkpoint. Returns list of dicts."""
...
def fetch_finance(start_date="2020-01-01", end_date=None, output=None,
    ...
    ----------
    workers:
        Number of parallel workers for month fetching (1 = serial).
        Default 6. All workers share the global rate limiter.
    cleanup:
        If True, remove per-month checkpoint files after consolidation.
    """
    ...
    if not months:
        log_print("[finance] No months in range")
        return pd.DataFrame()

    log_print(f"[finance] {len(months)} months: "
              f"{months[0][0]} ~ {months[-1][1]}")

    checkpoints_dir = Path(DATA_DIR) / "finance"
    checkpoints_dir.mkdir(exist_ok=True)

    # ── Phase 1: load cached checkpoints ──
    all_data = []
    todo = []
    for (r_start, r_end) in months:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[finance] {label} → checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[finance] {label} → checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append((r_start, r_end, label))

    if not todo:
        log_print(f"[finance] All {len(months)} months already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[finance] {len(todo)} to fetch, workers={workers}, "
    ...
    if not df.empty and 'stock_code' in df.columns:
        ...
        if len(df) < before:
            log_print(f"[finance] Dropped {before - len(df)} rows with malformed stock_code")
    ...
    log_print(f"[finance] Saved → {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[finance] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch stock daily finance data")
