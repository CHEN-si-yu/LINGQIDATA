...
def _fetch_range(start_time, end_time, api_key):
    ...
    log_print(f"  [fund_flow] splitting {start_time}~{end_time} "
...
def fetch_main_fund_flow(start_date="2019-01-01", end_date=None, output=None,
                         resume=True, workers=6, cleanup=True):
    """Fetch main fund flow (大/中/小/特大单) for all stocks."""
    ...
    if not months:
        log_print("[fund_flow] No months in range")
        return pd.DataFrame()

    log_print(f"[fund_flow] {len(months)} months: "
              f"{months[0][0]} ~ {months[-1][1]}")

    checkpoints_dir = Path(DATA_DIR) / "main_fund_flow"
    ...
    for (r_start, r_end) in months:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[fund_flow] {label} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[fund_flow] {label} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append((r_start, r_end, label))

    if not todo:
        log_print(f"[fund_flow] All {len(months)} months already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[fund_flow] {len(todo)} to fetch, workers={workers}, "
    ...
    if not df.empty and 'stock_code' in df.columns:
        ...
        if len(df) < before:
            log_print(f"[fund_flow] Dropped {before - len(df)} rows with malformed stock_code")
    ...
    log_print(f"[fund_flow] Total: {len(df)} rows, {len(df.columns)} columns")

    if output is None:
        output = f"{DATA_DIR}/main_fund_flow.parquet"
    df.to_parquet(output, index=False)
    log_print(f"[fund_flow] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[fund_flow] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch main fund flow data")
