...
def _fetch_range(start_time, end_time, api_key):
    ...
    log_print(f"  [margin_detail] splitting {start_time}~{end_time} "
...
def fetch_margin_detail(start_date="2019-01-01", end_date=None, output=None,
    ...
    generates ~8-9M rows over 7 years.  Monthly checkpoints and
    recursive date-range splitting handle the volume safely.
    """
    ...
    if not months:
        log_print("[margin_detail] No months in range")
        return pd.DataFrame()

    log_print(f"[margin_detail] {len(months)} months: "
              f"{months[0][0]} ~ {months[-1][1]}")

    checkpoints_dir = Path(DATA_DIR) / "margin_detail"
    ...
    for (r_start, r_end) in months:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[margin_detail] {label} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[margin_detail] {label} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append((r_start, r_end, label))

    if not todo:
        log_print(f"[margin_detail] All {len(months)} months already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[margin_detail] {len(todo)} to fetch, workers={workers}, "
                ...
                for fut in as_completed(futures):
                    ...
                    try:
                        ...
                        with lock:
                            ...
                            completed += 1
                        log_print(f"[margin_detail] [{completed}/{len(todo)}] {label} -> "
                                  f"{len(batch)} rows | tokens={limiter.stats['tokens_available']}")
                    except Exception as e:
                        with lock:
                            completed += 1
                        log_print(f"[margin_detail] [{completed}/{len(todo)}] {label} FAILED: {e}")
        else:
            for i, (rs, re, label) in enumerate(todo):
                batch = _fetch_and_save(rs, re, label, api_key, checkpoints_dir)
                if batch:
                    all_data.extend(batch)
                log_print(f"[margin_detail] [{i+1}/{len(todo)}] {label} -> "
                          f"{len(batch)} rows | tokens={limiter.stats['tokens_available']}")
    ...
    if not df.empty and 'stock_code' in df.columns:
        ...
        if len(df) < before:
            log_print(f"[margin_detail] Dropped {before - len(df)} rows with malformed stock_code")
    ...
    log_print(f"[margin_detail] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[margin_detail] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch margin trading detail data")
