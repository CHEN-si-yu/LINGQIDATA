"""
Prepare training data for V2 model — with strict point-in-time alignment.
...
def parse_args():
    p = argparse.ArgumentParser(description="Prepare V2 training data")
    p.add_argument("--source-root", type=str, default=None,
                   help="featureengineering/data directory")
    p.add_argument("--output-root", type=str, default=None,
                   help="Output root for trainingdata/v2/")
...
def main():
    ...
    out = Path(args.output_root) if args.output_root else Path("/root/shared-nvme/lingqiData/trainingdata/v2")
    ...
    # ── Build trade_amt ────────────────────────────────────────────────
    daily_path = Path("/root/shared-nvme/lingqiData/data/daily_adj.parquet")
    ...
    print(f"\n{'='*70}")
    print(f"  V2 TRAINING DATA SUMMARY  ({elapsed:.0f}s total)")
    print(f"{'='*70}")
