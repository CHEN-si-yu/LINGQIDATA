...
def _fetch_page(start_date, end_date, page, page_size, api_key, retries=3):
...
def _fetch_month(start_date, end_date, api_key):
    all_data = []
    page = 0
    page_size = 10000

    while True:
        batch, total = _fetch_page(start_date, end_date, page, page_size, api_key)
        if not batch:
            break
...
def _fetch_and_save(start_date, end_date, label, api_key, checkpoints_dir):
    batch = _fetch_month(start_date, end_date, api_key)
    if batch:
...
def fetch_report_rc(start_date="2019-01-01", end_date=None, output=None,
                    resume=True, workers=6, cleanup=True):
    """Fetch broker research report earnings forecasts."""
    ...
    if not months:
        log_print("[report_rc] No months in range")
        return pd.DataFrame()

    log_print(f"[report_rc] {len(months)} months: "
              f"{months[0][0]} ~ {months[-1][1]}")

    checkpoints_dir = Path(DATA_DIR) / "report_rc"
    ...
    for (r_start, r_end) in months:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[report_rc] {label} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[report_rc] {label} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append((r_start, r_end, label))

    if not todo:
        log_print(f"[report_rc] All {len(months)} months already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[report_rc] {len(todo)} to fetch, workers={workers}, "
    ...
    if not df.empty and 'stock_code' in df.columns:
        ...
        if len(df) < before:
            log_print(f"[report_rc] Dropped {before - len(df)} rows with malformed stock_code")

        _key = df['stock_code'].str.extract(r'(\d{6})').iloc[:, 0]
        df['_sort_code'] = pd.to_numeric(_key, errors='coerce').fillna(0).astype(int)
        df = df.sort_values(['_sort_code', 'report_date']).drop(columns=['_sort_code']).reset_index(drop=True)

    log_print(f"[report_rc] Total: {len(df)} rows, {len(df.columns)} columns")

    if output is None:
        output = f"{DATA_DIR}/report_rc.parquet"
    df.to_parquet(output, index=False)
    log_print(f"[report_rc] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[report_rc] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch broker research report forecasts")
