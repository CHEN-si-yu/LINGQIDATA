...
def fetch_cashflow(start_date="2019-01-01", end_date=None, output=None,
    ...
    if not quarters:
        log_print("[cashflow] No quarters in range")
        return pd.DataFrame()

    log_print(f"[cashflow] {len(quarters)} quarters: {quarters[0]} ~ {quarters[-1]}")

    checkpoints_dir = Path(DATA_DIR) / "cashflow"
    ...
    for q_end in quarters:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[cashflow] {q_end} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[cashflow] {q_end} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append(q_end)

    if not todo:
        log_print(f"[cashflow] All {len(quarters)} quarters already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[cashflow] {len(todo)} to fetch, workers={workers}, "
    ...
    if not df.empty and 'stock_code' in df.columns:
        ...
        if len(df) < before:
            log_print(f"[cashflow] Dropped {before - len(df)} rows with malformed stock_code")
    ...
    log_print(f"[cashflow] Total: {len(df)} rows, {len(df.columns)} columns")

    if output is None:
        output = f"{DATA_DIR}/cashflow.parquet"
    df.to_parquet(output, index=False)
    log_print(f"[cashflow] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[cashflow] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch stock cashflow statement data")
