...
def _fetch_range(start_time, end_time, api_key):
    ...
    if start_time == end_time:
        log_print(f"  [pledge_stat] {start_time} total={total} > 100K, "
    ...
    if mid == start_dt:
        log_print(f"  [pledge_stat] {start_time}~{end_time} total={total} > 100K, "
                  f"fetching first 100K rows")
        all_data = list(page0)
        page = 1
        while page * 10000 <= 100000:
    ...
    log_print(f"  [pledge_stat] splitting {start_time}~{end_time} "
...
def fetch_pledge_stat(start_date="2019-01-01", end_date=None, output=None,
                      resume=True, workers=6, cleanup=True):
    """Fetch stock pledge statistics (pledge_stat) for all stocks, quarterly."""
    ...
    if not months:
        log_print("[pledge_stat] No months in range")
        return pd.DataFrame()

    log_print(f"[pledge_stat] {len(months)} months: "
              f"{months[0][0]} ~ {months[-1][1]}")

    checkpoints_dir = Path(DATA_DIR) / "pledge_stat"
    ...
    for (r_start, r_end) in months:
        ...
        if resume and checkpoint_file.exists():
            try:
                ...
                log_print(f"[pledge_stat] {label} -> checkpoint ({len(df_q)} rows)")
                continue
            except Exception:
                log_print(f"[pledge_stat] {label} -> checkpoint corrupt, re-fetching")
                checkpoint_file.unlink(missing_ok=True)
        todo.append((r_start, r_end, label))

    if not todo:
        log_print(f"[pledge_stat] All {len(months)} months already cached")
    else:
        limiter = rate_limiter()
        log_print(f"[pledge_stat] {len(todo)} to fetch, workers={workers}, "
    ...
    if not df.empty and 'stock_code' in df.columns:
        ...
        if len(df) < before:
            log_print(f"[pledge_stat] Dropped {before - len(df)} rows with malformed stock_code")
    ...
    log_print(f"[pledge_stat] Total: {len(df)} rows, {len(df.columns)} columns")

    if output is None:
        output = f"{DATA_DIR}/pledge_stat.parquet"
    df.to_parquet(output, index=False)
    log_print(f"[pledge_stat] Saved -> {output}")
    ...
    if cleanup:
        ...
        try:
            ...
            pass
        log_print(f"[pledge_stat] Cleaned up {removed} checkpoint files")

    return df


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch stock pledge statistics")
